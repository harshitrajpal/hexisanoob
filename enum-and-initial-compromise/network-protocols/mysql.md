...
```shell-session
harshitp0tter@htb[/htb]$ sudo apt install mysql-server -y
harshitp0tter@htb[/htb]$ cat /etc/mysql/mysql.conf.d/mysqld.cnf | grep -v "#" | sed -r '/^\s*$/d'
...
```shell-session
harshitp0tter@htb[/htb]$ sudo nmap 10.129.14.128 -sV -sC -p3306 --script mysql*
...
```shell-session
harshitp0tter@htb[/htb]$ mysql -u root -h 10.129.14.132
...
```shell-session
harshitp0tter@htb[/htb]$ mysql -u root -pP4SSw0rd -h 10.129.14.128
