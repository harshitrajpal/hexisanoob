...
1. User enumeration:
   1. smtp\_enum module in metasploit (very nice)
   2. [https://nmap.org/nsedoc/scripts/smtp-enum-users.html](https://nmap.org/nsedoc/scripts/smtp-enum-users.html) (not so nice)
   3. smtp-user-enum python script ([https://github.com/cytopia/smtp-user-enum](https://github.com/cytopia/smtp-user-enum)) (very very nice)
2. Other footprinting mechanisms:
   1. `find / -type f -name smtp* 2>/dev/null | grep nmap`
   2. Use the scripts:\
      sudo nmap -p smtp \<IP> --script=smtp\*
...
```shell-session
harshitp0tter@htb[/htb]$ telnet 10.129.14.128 25
...
```shell-session
harshitp0tter@htb[/htb]$ sudo nmap 10.129.14.128 -sC -sV -p25
...
```shell-session
harshitp0tter@htb[/htb]$ sudo nmap 10.129.14.128 -p25 --script smtp-open-relay -v
