...
```shell-session
harshitp0tter@htb[/htb]$ dig soa www.inlanefreight.com
...
```shell-session
harshitp0tter@htb[/htb]$ dig ns inlanefreight.htb @10.129.14.128
...
```shell-session
harshitp0tter@htb[/htb]$ dig CH TXT version.bind 10.129.120.85
...
```shell-session
harshitp0tter@htb[/htb]$ dig any inlanefreight.htb @10.129.14.128
...
```shell-session
harshitp0tter@htb[/htb]$ dig axfr inlanefreight.htb @10.129.14.128
...
```shell-session
harshitp0tter@htb[/htb]$ dig axfr internal.inlanefreight.htb @10.129.14.128
...
```shell-session
harshitp0tter@htb[/htb]$ for sub in $(cat /opt/useful/seclists/Discovery/DNS/subdomains-top1million-110000.txt);do dig $sub.inlanefreight.htb @10.129.14.128 | grep -v ';\|SOA' | sed -r '/^\s*$/d' | grep $sub | tee -a subdomains.txt;done
...
```shell-session
harshitp0tter@htb[/htb]$ dnsenum --dnsserver 10.129.14.128 --enum -p 0 -s 0 -o subdomains.txt -f /opt/useful/seclists/Discovery/DNS/subdomains-top1million-110000.txt inlanefreight.htb
