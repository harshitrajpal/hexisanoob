...
```shell-session
harshitp0tter@htb[/htb]$ cat /etc/snmp/snmpd.conf | grep -v "#" | sed -r '/^\s*$/d'
...
```shell-session
harshitp0tter@htb[/htb]$ snmpwalk -v2c -c public 10.129.14.128
...
```shell-session
harshitp0tter@htb[/htb]$ sudo apt install onesixtyone
harshitp0tter@htb[/htb]$ onesixtyone -c /opt/useful/seclists/Discovery/SNMP/snmp.txt 10.129.14.128
...
```shell-session
harshitp0tter@htb[/htb]$ sudo apt install braa
harshitp0tter@htb[/htb]$ braa <community string>@<IP>:.1.3.6.*   # Syntax
harshitp0tter@htb[/htb]$ braa public@10.129.14.128:.1.3.6.*
